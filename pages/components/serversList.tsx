        ...
        <div id="cuerpo" className="lista-servers-vpns col-span-12 xl:col-span-9 w-full h-full flex flex-col gap-4">
...

            {
                data.results.map((d) => (
                    ...
                    <div className="item-server-vpn w-full h-24 sm:h-28 bg-white dark:bg-[#212121] dark:text-[#cfcfcf] rounded-lg shrink-0 flex items-center justify-between px-4 py-2 md:py-4 shadow-lg hover:cursor-pointer hover:-translate-y-[2px] dark:hover:bg-[#303030] hover:bg-[#FDFDFD] transition-all ">
                        ...
                        <div className="data-country-ip gap-1 flex flex-col items-start justify-center">
                            <div className="w-auto h-auto flex gap-2 items-center">
                                <div className="country-flag font-twemoji select-none flex items-center text-3xl sm:text-4xl w-auto h-6 cursor-pointer">{getFlagEmoji(d.location_country_code)}</div>
                                <div className="nombre-ip w-auto h-auto flex flex-col ">
                                    <span className=" leading-[1.10rem] font-semibold w-auto overflow-hidden truncate text-[14px] sm:text-[16px] ">{d.location_country}</span>
                                    <span className=" leading-[1.10rem] font-medium w-auto overflow-hidden truncate text-[14px] text-[#7e7d7d] sm:text-[16px] ">{d.ip_address}...</span>
                                </div>
                            </div>
                            ...
                            <div className=" w-auto h-auto flex gap-2">
                                <div className=" hidden relative w-11 h-6 flex items-center justify-start select-none bg-[#cfcfcf] font-medium text-xs sm:text-sm rounded overflow-hidden">
                                    <div className={"w-full h-full" + ((getPercentage(d.times_check_succeeded, d.times_checked) <= 79) && (getPercentage(d.times_check_succeeded, d.times_checked) > 30) && 'bg-yellow-500 w-6') || ((getPercentage(d.times_check_succeeded, d.times_checked) > 79) && 'bg-green-500 w-full') || ((getPercentage(d.times_check_succeeded, d.times_checked) <= 30) && 'bg-red-500 w-3')}></div>
                                    <span className="absolute w-12 h-6 z-10 text-[#303030] flex items-center justify-center px-2">{getPercentage(d.times_check_succeeded, d.times_checked)}</span>
                                </div>

                                <div className="w-14 h-6 before:content-[''] before:absolute before:rounded flex gap-1 items-center justify-start select-none bg-[#cfcfcf] font-medium text-xs sm:text-sm rounded overflow-visible" >
                                    <div className="w-full h-full flex items-center justify-center z-10 gap-0.5 ">
                                        ...
                                        <span className="w-auto h-6 text-[#303030] flex items-center justify-center"></span>

                                    </div>
                                </div>

                                <div className="w-14 h-6 before:content-[''] before:absolute before:rounded before:w-14 before:h-6 before:bg-[#7474FF] flex gap-1 items-center justify-start select-none bg-[#cfcfcf] font-medium text-xs sm:text-sm rounded overflow-visible" >
                                    <div className="w-full h-full flex items-center justify-center z-10 gap-0.5 ">
                                        ...
                                        <span className="w-auto h-6 text-white text-xs sm:text-sm font-semibold flex items-center justify-center overflow-hidden"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <button title="Copy" className="opt-copiar relative w-4 h-4 sm:w-10 sm:h-10 rounded-md flex items-center justify-center cursor-pointer hover:bg-[#EBEBEB] dark:hover:bg-[#1B1B1B] active:bg-[#D7D7D7] dark:active:bg-[#111111] transition-colors transition-transform active:scale-[95%]">
                        <div className="absolute w-4 h-4 sm:w-10 sm:h-10 rounded-md flex items-center justify-center" x-show="timeCopy, setTimeout(() => { timeCopy = false }, 500)">
                        ...
                        <svg className="fill-[#303030] dark:fill-[#cfcfcf]" x-show="!timeCopy" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <button title="QR Code" className="opt-codigo-qr w-4 h-4 sm:w-10 sm:h-10 rounded-md flex items-center justify-center cursor-pointer hover:bg-[#EBEBEB] dark:hover:bg-[#1B1B1B] active:bg-[#D7D7D7] dark:active:bg-[#111111] transition-colors transition-transform active:scale-[95%]">
                    ...
                    {/* <a> */}
                    <button title="Download" className="opt-descargar w-4 h-4 sm:w-10 sm:h-10 rounded-md flex items-center justify-center cursor-pointer hover:bg-[#EBEBEB] dark:hover:bg-[#1B1B1B] active:bg-[#D7D7D7] dark:active:bg-[#111111] transition-colors transition-transform active:scale-[95%]">
